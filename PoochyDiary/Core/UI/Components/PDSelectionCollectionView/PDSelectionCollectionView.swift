...
class PDSelectionCollectionView: BaseView {
    ...
    func configure(selectedId: String) {
        guard let model,
            let visibleCells = collectionView.visibleCells as? [PDSelectionCollectionViewCell]
        else {
            return
        }
...
extension PDSelectionCollectionView: UICollectionViewDelegate {
    func collectionView(
        ...
        guard let model,
            let selectedItem = model.items[safe: indexPath.item]
        else {
            return
        }
        ...
        DataSource(collectionView: collectionView) { [weak self] collectionView, indexPath, item in
            ...
            guard
                let cell = collectionView.dequeueReusableCell(
                    ...
                    for: indexPath) as? PDSelectionCollectionViewCell
            else {
                return nil
            }
