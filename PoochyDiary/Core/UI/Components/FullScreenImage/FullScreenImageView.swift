...
class FullScreenImageView: BaseView {
    ...
    override func constructSubviewLayoutConstraints() {
        ...
        NSLayoutConstraint.activate([
            ...
            pageLabel.bottomAnchor.constraint(equalTo: safeAreaLayoutGuide.bottomAnchor, constant: -Spacing.space16),
    ...
    var currentDisplayedImage: UIImage? {
        ...
        return (collectionView.cellForItem(at: IndexPath(item: clamped, section: 0)) as? FullScreenImageViewCell)?.currentImage
    }
    ...
    func startItemImageFrame(in targetView: UIView, fallbackImageSize: CGSize? = nil) -> CGRect? {
        itemImageFrame(for: model?.startIndex ?? 0, in: targetView, fallbackImageSize: fallbackImageSize)
    }
    ...
    private func makeDataSource() -> DataSource {
        DataSource(collectionView: collectionView) { collectionView, indexPath, photo in
            guard let cell = collectionView.dequeueReusableCell(
                ...
                for: indexPath
            ) as? FullScreenImageViewCell else { return nil }
