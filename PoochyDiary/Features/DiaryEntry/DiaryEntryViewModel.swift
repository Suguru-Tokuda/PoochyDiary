...
class DiaryEntryViewModel {
    ...
    func addPhoto(image: UIImage) {
        ...
        state.photos.append(Photo(
            id: UUID(),
            fileName: "\(pet.id.uuidString)-\(state.diaryId.uuidString)-photo-\(createdAt.description.replacingOccurrences(of: " ", with: "_"))",
            image: image,
            createdAt: createdAt,
    ...
    func save(completion: @escaping(Result<Void, Error>) -> ()) {
        ...
        saveTask = Task(priority: .userInitiated) { [weak self] in
            ...
            do {
                try await coreDataManager.saveDiary(diary:
                                                Diary(id: state.diaryId,
                                                        petId: pet.id,
