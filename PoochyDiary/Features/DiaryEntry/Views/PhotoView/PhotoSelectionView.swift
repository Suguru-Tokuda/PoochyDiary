...
class PhotoSelectionView: BaseView {
  ...
  private let stackView = UIStackView(
    axis: .vertical, alignment: .fill, distribution: .fill, spacing: Spacing.space12)
  ...
  override func constructSubviews() {
    ...
    imageGalleryButton.addTarget(
      self, action: #selector(handleImageGalleryButtonTap), for: .touchUpInside)
  }
    ...
    DataSource(collectionView: collectionView) { collectionView, indexPath, item in
      guard
        let cell = collectionView.dequeueReusableCell(
          withReuseIdentifier: PhotoSelectionCollectionViewCell.reuseIdentifier, for: indexPath)
          as? PhotoSelectionCollectionViewCell
      else {
        return nil
      }
