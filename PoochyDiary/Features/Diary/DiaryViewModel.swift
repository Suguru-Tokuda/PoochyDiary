...
class DiaryViewModel {
    ...
    private func updateDisplayedDates() {
        ...
        guard let visibleWeekStart = calendar.dateInterval(
            of: .weekOfYear,
            for: visibleWeekDate
        )?.start else {
            return
...
extension DiaryViewModel {
    ...
    static func generateMockData() -> [Diary] {
        ...
        func note(stoolType: StoolType, mucusLevel: MucusLevel, bloodAmount: BloodAmount, using rng: inout SeededGenerator) -> String {
            var parts: [String] = []
            ...
            if bloodAmount != .none {
                parts.append("Saw a \(bloodAmount.name.lowercased()) amount of blood — will monitor over the next day or two.")
            }
                ...
                for i in 0..<entryCount {
                    ...
                    diaries.append(
                        Diary(
                            ...
                            note: note(stoolType: stoolType, mucusLevel: mucusLevel, bloodAmount: bloodAmount, using: &rng),
